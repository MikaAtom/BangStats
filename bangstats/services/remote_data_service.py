...
class RemoteDataService:
    ...
    def __init__(self, server: str = "en"):
        ...
        config = Config()
        self.cache_folder_path = config.get('REMOTE_DATA_CACHE_PATH')
        ...
        self.remote_request_retries = config.get('REMOTE_REQUEST_RETRIES')
        self.remote_request_timeout = config.get('REMOTE_REQUEST_TIMEOUT')
        self.server_id = config.get('SERVERS').get(server, 1)

    def _with_retry(self, func: Callable, retries: int = None, delay: int = None):  # pragma: no cover
    ...
    def _get_remote_songs_ids(self) -> List[dict]:
        ...
        def fetch():
            ...
            response = requests.get(self.SONGS_ALL_API)
            logger.debug(f"Received response with status {response.status_code} for songs list")
            if response.status_code == 200:
                songs_data = response.json()
                logger.debug(f"Fetched {len(songs_data)} songs from remote API")
                return songs_data
            else:
                raise Exception(f"Failed to fetch songs. Status code: {response.status_code}")
    ...
    def _get_remote_song_info(self, song_id: str) -> Dict[str, Any]:
        ...
        def fetch():
            ...
            logger.debug(f"Received response with status {response.status_code} for song {song_id}")
            if response.status_code == 200:
                logger.debug(f"Fetched song info for {song_id} from remote API")
                return response.json()
            else:
                raise Exception(f"Failed to fetch song info. Status code: {response.status_code}")
    ...
    def _get_remote_bands_info(self) -> Dict[str, Any]:
        ...
        def fetch():
            ...
            response = requests.get(self.BANDS_API)
            logger.debug(f"Received response with status {response.status_code} for bands info")
            if response.status_code == 200:
                logger.debug("Fetched bands info from remote API")
                return response.json()
            else:
                raise Exception(f"Failed to fetch bands info. Status code: {response.status_code}")
    ...
    def _get_remote_events_info(self) -> Dict[str, Any]:
        ...
        def fetch():
            ...
            response = requests.get(self.EVENTS_API)
            logger.debug(f"Received response with status {response.status_code} for events info")
            if response.status_code == 200:
                logger.debug("Fetched events info from remote API")
                return response.json()
            else:
                raise Exception(f"Failed to fetch events info. Status code: {response.status_code}")
    ...
    def _get_cache_song_info(self, song_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            matching_files = [f for f in os.listdir(self.cache_songs_folder_path)
    ...
    def get_song_info(self, song_id: str, force_remote: bool = False) -> Optional[Dict[str, Any]]:
        """
