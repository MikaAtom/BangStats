...
class SongService:
    ...
    def create_song(self, song_data: Dict[str, Any]) -> Song:
        ...
        if self.get_song_by_internal_id(song_data["internal_song_id"]):
            raise ValueError(f"Song with internal_song_id {song_data['internal_song_id']} already exists")

        if not isinstance(song_data["internal_song_id"], int) or song_data["internal_song_id"] <= 0:
    ...
    def search_songs_by_name(self, name_query: str, language: str = "en") -> List[Song]:
        ...
        logger.debug(f"Searching songs by name '{name_query.strip()}' in language '{language}'")
    ...
    def get_songs_by_difficulty(self, difficulty: str, level: int) -> List[Song]:
        ...
        if difficulty not in valid_difficulties:
            logger.warning(f"Invalid difficulty for get_songs_by_difficulty: {difficulty}")
            return []
    ...
    def get_songs_by_note_count(self, note_count: int, difficulty: Optional[str] = None) -> List[Song]:
        ...
        if difficulty:
            valid_difficulties = ["easy", "normal", "hard", "expert", "special"]
            if difficulty not in valid_difficulties:
                logger.warning(f"Invalid difficulty for get_songs_by_note_count: {difficulty}")
                return []
            logger.debug(f"Fetching songs by note_count {note_count} and difficulty '{difficulty}'")
    ...
    def get_song_statistics(self) -> Dict[str, Any]:
        ...
        band_counts = {}
        difficulty_counts = {"easy": 0, "normal": 0, "hard": 0, "expert": 0, "special": 0}
        tag_counts = {}
    ...
    def validate_song_data(self, song_data: Dict[str, Any], is_update: bool = False) -> Tuple[bool, Optional[str]]:
        """
