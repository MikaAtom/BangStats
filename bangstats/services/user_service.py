...
class UserService:
    ...
    def create_user(self, user_data: Dict[str, Any]) -> User:
        ...
        if "screenshots_source" in user_data:
            if not isinstance(user_data["screenshots_source"], str) or not user_data["screenshots_source"].strip():
    ...
    def update_user(self, user_id: int, user_data: Dict[str, Any]) -> User:
        ...
        if "game_id" in user_data and (not isinstance(user_data["game_id"], str) or not user_data["game_id"]):
            raise ValueError("game_id must be a non-empty string")
        if "username" in user_data and (not isinstance(user_data["username"], str) or not user_data["username"]):
            raise ValueError("username must be a non-empty string")
        if "server" in user_data and (not isinstance(user_data["server"], str) or not user_data["server"]):
