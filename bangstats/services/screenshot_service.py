...
from bangstats.database.models.screenshot import Screenshot
...
class ScreenshotService:
    ...
    def __init__(self):
        ...
        logger.debug("ScreenshotService initialized with ScreenshotRepository and SongService")
    ...
    def get_screenshots_by_song(self, user_id: int, song_id: int, difficulty: Optional[str] = None) -> List[Screenshot]:
        if not isinstance(user_id, int) or user_id <= 0:
            logger.warning(f"Invalid user_id provided: {user_id}")
            return []
        ...
        if difficulty:
            if not isinstance(difficulty, str):
                ...
                return []
            logger.debug(f"Fetching screenshots for user {user_id} and song {song_id} with difficulty {difficulty}")
        ...
        logger.debug(f"Fetching screenshots for user {user_id} and song {song_id} across difficulties: {difficulties}")
    ...
    def create_screenshot(self, screenshot_data: Dict[str, Any]) -> Screenshot:
        """
        ...
        required = ["user_id", "score", "live_type", "perfect", "great", "good", "bad", "miss", "max_combo", "song_id", "difficulty"]
        ...
        for field in note_fields:
            if field in screenshot_data and (not isinstance(screenshot_data[field], int) or screenshot_data[field] < 0):
    ...
    def update_screenshot(self, screenshot_id: int, screenshot_data: Dict[str, Any]) -> Screenshot:
        ...
        """
        logger.debug(f"Attempting to update screenshot {screenshot_id} with data: {screenshot_data}")
    ...
    def get_screenshots_by_difficulty(self, user_id: int, difficulty: Optional[str] = None) -> List[Screenshot]:
        ...
        if difficulty:
            if not isinstance(difficulty, str):
                logger.warning(f"Invalid difficulty provided: {difficulty}")
                return []
            logger.debug(f"Fetching screenshots for user {user_id} and difficulty: {difficulty}")
            return self._repo.get_by_difficulty(user_id, difficulty)

        # Get all available difficulties
        difficulties = self._get_available_difficulties()
        logger.debug(f"Fetching screenshots for user {user_id} across difficulties: {difficulties}")
    ...
    def get_full_combos_by_user(self, user_id: int, difficulty: Optional[str] = None) -> List[Screenshot]:
        ...
        if difficulty:
            if not isinstance(difficulty, str):
                logger.warning(f"Invalid difficulty provided: {difficulty}")
                return []
            logger.debug(f"Fetching full combos for user {user_id} with difficulty {difficulty}")
            return self._repo.get_full_combos(user_id, difficulty)

        # Get all available difficulties
        difficulties = self._get_available_difficulties()
        logger.debug(f"Fetching full combos for user {user_id} across difficulties: {difficulties}")
    ...
    def get_all_perfects_by_user(self, user_id: int, difficulty: Optional[str] = None) -> List[Screenshot]:
        ...
        if difficulty:
            if not isinstance(difficulty, str):
                logger.warning(f"Invalid difficulty provided: {difficulty}")
                return []
            logger.debug(f"Fetching all perfects for user {user_id} with difficulty {difficulty}")
            return self._repo.get_all_perfects(user_id, difficulty)

        # Get all available difficulties
        difficulties = self._get_available_difficulties()
        logger.debug(f"Fetching all perfects for user {user_id} across difficulties: {difficulties}")
    ...
    def get_high_scores_by_user(self, user_id: int, difficulty: Optional[str] = None) -> List[Screenshot]:
        ...
        if difficulty:
            if not isinstance(difficulty, str):
                logger.warning(f"Invalid difficulty provided: {difficulty}")
                return []
            logger.debug(f"Fetching high scores for user {user_id} with difficulty {difficulty}")
            return self._repo.get_high_scores_by_user(user_id, difficulty)

        # Get all available difficulties
        difficulties = self._get_available_difficulties()
        logger.debug(f"Fetching high scores for user {user_id} across difficulties: {difficulties}")
    ...
    def get_anomalies_by_user(self, user_id: int, difficulty: Optional[str] = None) -> List[Screenshot]:
        ...
        if difficulty:
            if not isinstance(difficulty, str):
                logger.warning(f"Invalid difficulty provided: {difficulty}")
                return []
            logger.debug(f"Fetching anomalies for user {user_id} with difficulty {difficulty}")
            return self._repo.get_anomalies(user_id, difficulty)

        # Get all available difficulties
        difficulties = self._get_available_difficulties()
        logger.debug(f"Fetching anomalies for user {user_id} across difficulties: {difficulties}")
    ...
    def get_screenshot_by_timestamp(self, user_id: int, timestamp: datetime, difficulty: Optional[str] = None) -> Optional[Screenshot]:
        ...
        if difficulty:
            if not isinstance(difficulty, str):
                logger.warning(f"Invalid difficulty provided: {difficulty}")
                return None
            logger.debug(f"Fetching screenshot for user {user_id} at timestamp {timestamp} with difficulty {difficulty}")
            return self._repo.get_by_timestamp(user_id, timestamp, difficulty)

        # Get all available difficulties
        difficulties = self._get_available_difficulties()
        logger.debug(f"Fetching screenshot for user {user_id} at timestamp {timestamp} across difficulties: {difficulties}")
    ...
    def get_screenshots_by_timestamp_interval(self, user_id: int, start_time: datetime, end_time: datetime, difficulty: Optional[str] = None) -> List[Screenshot]:
        ...
        if start_time >= end_time:
            logger.warning(f"Invalid timestamp interval: start_time must be before end_time")
            return []
        ...
        if difficulty:
            if not isinstance(difficulty, str):
                logger.warning(f"Invalid difficulty provided: {difficulty}")
                return []
            logger.debug(f"Fetching screenshots for user {user_id} between {start_time} and {end_time} with difficulty {difficulty}")
            return self._repo.get_by_timestamp_interval(user_id, start_time, end_time, difficulty)

        # Get all available difficulties
        difficulties = self._get_available_difficulties()
        logger.debug(f"Fetching screenshots for user {user_id} between {start_time} and {end_time} across difficulties: {difficulties}")
