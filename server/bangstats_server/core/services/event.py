...
class EventService(BaseCRUDService):
    ...
    def get_event_by_event_id(self, event_id: int) -> Optional[Event]:
        if not isinstance(event_id, int) or event_id <= 0:
            logger.warning(
                f"Invalid event_id provided for get_event_by_event_id: {event_id}"
            )
            return None
    ...
    def create_event(self, event_data: Dict[str, Any]) -> Event:
        ...
        if not isinstance(event_data["event_start_at"], dict) or not isinstance(
            event_data["event_end_at"], dict
        ):
    ...
    def update_event(self, event_id: int, event_data: Dict[str, Any]) -> Event:
        ...
        if "event_id" in event_data:
            if (
                not isinstance(event_data["event_id"], int)
                or event_data["event_id"] <= 0
            ):
                raise ValueError("event_id must be a positive integer")
        ...
        if "event_start_at" in event_data and not isinstance(
            event_data["event_start_at"], dict
        ):
            raise ValueError("event_start_at must be a dictionary")

        if "event_end_at" in event_data and not isinstance(
            event_data["event_end_at"], dict
        ):
    ...
    def search_events_by_name(self, name_query: str, language: str = "en") -> List[Event]:
        ...
        if not isinstance(language, str):
            logger.warning(
                "Invalid language for search_events_by_name, defaulting to 'en'"
            )
            language = "en"

        logger.debug(
            f"Searching events by name '{name_query.strip()}' in language '{language}'"
        )
    ...
    def search_events_by_date(
        ...
        if isinstance(timestamp, str):
            try:
                ...
                logger.warning(
                    f"Invalid timestamp string for search_events_by_date: {timestamp}"
                )
                return None
    ...
    def get_current_event(self, language: str = "en") -> Optional[Event]:
        ...
        logger.debug(
            f"Fetching current event at timestamp {current_time} in language '{language}'"
        )
